...
2. **Propose-then-apply**: find issues, list them in logical batches, get an OK per
   batch (unless `apply`).
3. Fixes: broken/relative links, duplicate pages (merge conservatively — a bad merge
   is hard to undo), overloaded pages (split), stale pages (**archive, never
   hard-delete**), and a refreshed `wiki/index.md`.
4. Log a maintenance line to `wiki/log.md`.
